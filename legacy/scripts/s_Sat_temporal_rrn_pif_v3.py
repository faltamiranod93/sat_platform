...
def robust_transform_from_class_medians(
    ...
    if len(rows) >= 2:
        ...
        # Guard: gain negativo es físicamente imposible en reflectancia
        if gain <= 0:
            print(f"  [WARN] Gain negativo ({gain:.4f}) descartado para banda. Usando offset-only fallback.")
            offset = float(np.average(y - x, weights=w))
            gain = 1.0
            info["mode"] = "offset_only_fallback_negative_gain"
        else:
...
def main():
    ...
    for _, row in roi_list.iterrows():
        ...
        print(f"[LOAD] {fecha} — {tif_path.name}")
        arr, meta = load_tif(tif_path)
        print(f"       {meta['height']}x{meta['width']}px, {meta['count']} bandas")

        if meta["count"] < len(band_names):
            raise ValueError(
                f"TIF {tif_path.name} tiene solo {meta['count']} bandas, "
                f"se esperaban >= {len(band_names)}"
            )
        ...
        print(f"       Muestras: {len(ext)} puntos")
    ...
    for fecha in roi_list["Fecha"]:
        ...
        if fecha == args.reference_date:
            print(f"  -> Fecha de referencia, sin corrección.")
        ...
        for k, band in enumerate(band_names):
            ...
            print(f"  {band}: gain={gain:.4f}  offset={offset:.1f}  mode={info['mode']}")
