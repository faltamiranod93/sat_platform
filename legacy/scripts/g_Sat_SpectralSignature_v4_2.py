...
Principales características:
  - Convención Ng 0-based (Ng=0 == "no class"), coherente con config_bandas_v2/v3.
  - Carga flexible de config_bandas, lista ROI y MCAL desde diálogos.
  - Soporte para secuencias de bandas Nband_sort y Nband_filter (si existe en el JSON).
  - Firmas por píxel: fecha activa por defecto, con opción para ver todas las fechas.
  - Firmas promedio MCAL: promedio ± desviación estándar por grupo/fecha.
  - Firmas del MCAL: visor filtrable de firmas individuales del MCAL.
  - Exportación de firmas de sesión a CSV.
  - Guardado/carga de proyecto en JSON.
  - Exportación PNG de vista actual y guardado por lote.
"""
...
def _bands_from_legacy(cfg_old: Dict):
    ...
    band_sequences = {'Nband_sort': list(band_names)}
...
def reload_config(config_path: str):
    ...
    if 'bands' in cfg:
        (BAND_NAMES, LAMBDA_NM, BAND_INDEX,
         ...
         BAND_SEQUENCES) = _bands_from_legacy(cfg)
    else:
        (BAND_NAMES, LAMBDA_NM, BAND_INDEX,
         CLASSMAP_COLORS, PRESETS, GROUP_NAMES, GROUP_COLORS_HEX,
...
class ProjectState:
    ...
    pixel_xaxis_mode: str
    pixel_band_sequence: str
    pixel_all_dates: bool
...
def get_band_columns_from_df(df: pd.DataFrame, seq_name: Optional[str]) -> List[str]:
    return [b for b in get_band_sequence(seq_name) if b in df.columns]
...
class ImageCanvas(FigureCanvas):
    ...
    def set_mcal_points(self, df: pd.DataFrame, labels: List[str]):
        ...
        self._mcal_scatter = self.ax.scatter(
            xx, yy, s=20, c=colors, edgecolors='k', linewidths=0.3
        )
        self.draw_idle()
...
class SignatureCanvas(FigureCanvas):
    ...
    def plot_signature(
        ...
        if color is not None:
            line = self.ax.plot(
                xvals, yvals, marker=marker, linestyle=linestyle,
                linewidth=1.5, markersize=4, label=label, color=color
            )
            line_color = color
        else:
            line = self.ax.plot(
...
class SignatureWindow(QtWidgets.QDialog):
    def __init__(self, parent=None, title: str = 'Firmas espectrales', clear_callback=None, pixel_mode: bool = False):
        ...
        controls.addWidget(self.cmb_band_sequence)
...
class AverageSignatureWindow(QtWidgets.QDialog):
    ...
    def _connect_auto_refresh(self):
        self.chk_lambda.stateChanged.connect(lambda _=None: self._auto_refresh_if_needed())
        self.cmb_band_sequence.currentTextChanged.connect(lambda _=None: self._auto_refresh_if_needed())
        self.group_list.itemSelectionChanged.connect(self._auto_refresh_if_needed)
        self.date_list.itemSelectionChanged.connect(self._auto_refresh_if_needed)

    def _auto_refresh_if_needed(self):
        if self.isVisible() and self._has_plot:
            self.plot_average_signatures()
    ...
    def plot_average_signatures(self):
        ...
        if df_mcal.empty:
            QtWidgets.QMessageBox.warning(self, 'Advertencia', 'No hay datos MCAL disponibles.')
            return

        band_cols = get_band_columns_from_df(df_mcal, self.get_selected_band_sequence_name())
        if not band_cols:
        ...
        self.canvas.clear()
        self.canvas.set_band_labels(band_cols)
        xvals = (
            get_lambda_for_sequence(self.get_selected_band_sequence_name())
            if self.canvas.x_mode == 'lambda' else np.arange(len(band_cols))
        )
        ...
        for item in selected_items:
            ...
            for date_idx, date_str in enumerate(selected_dates):
                ...
                self.canvas.plot_signature(
                    xvals, mean_vals, label,
                    std_upper=upper_std, std_lower=lower_std,
                    color=color_hex, marker=marker
                )
                plots_created += 1
...
class McalSignatureWindow(QtWidgets.QDialog):
    def __init__(self, main_window: 'MainWindow', parent=None):
        ...
        self.info_label = QtWidgets.QLabel(
            ...
            'Para evitar que la aplicación se quede pensando, primero filtra por grupos y fechas, '
    ...
    def _build_layout(self):
        ...
        row.addWidget(self.cmb_band_sequence)
        left_layout.addLayout(row)
    ...
    def _connect_auto_refresh(self):
        self.chk_lambda.stateChanged.connect(lambda _=None: self._auto_refresh_if_needed())
        self.cmb_band_sequence.currentTextChanged.connect(lambda _=None: self._auto_refresh_if_needed())
        self.spn_max_records.valueChanged.connect(lambda _=None: self._auto_refresh_if_needed())
        self.group_list.itemSelectionChanged.connect(self._auto_refresh_if_needed)
        self.date_list.itemSelectionChanged.connect(self._auto_refresh_if_needed)

    def _auto_refresh_if_needed(self):
        if self.isVisible() and self._has_plot:
            self.plot_mcal_signatures()
    ...
    def plot_mcal_signatures(self):
        ...
        if filtered.empty:
            ...
            QtWidgets.QMessageBox.information(
                self, 'Información',
                'No hay registros MCAL para la combinación seleccionada.'
            )
            return
        ...
        for _, row in filtered.iterrows():
            ...
            ng_num = pd.to_numeric(row.get('Ng', np.nan), errors='coerce')
            ng = int(ng_num) if pd.notna(ng_num) else None
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _connect_aux_windows(self):
        ...
        self.sig_win.cmb_band_sequence.currentTextChanged.connect(lambda _=None: self._update_signatures_view())
    ...
    def _load_config_from_file(self):
        ...
        try:
            ...
            self._redraw_image()
            self._plot_mcal()
            if self.sig_win.isVisible() and self.selected_pixels:
                self._update_signatures_view()
            if self.avg_sig_win.isVisible() and getattr(self.avg_sig_win, '_has_plot', False):
                self.avg_sig_win.plot_average_signatures()
            if self.mcal_sig_win.isVisible() and getattr(self.mcal_sig_win, '_has_plot', False):
                self.mcal_sig_win.plot_mcal_signatures()
    ...
    def _update_signatures_view(self):
        ...
        xvals = (
            get_lambda_for_sequence(self.sig_win.get_selected_band_sequence_name())
            if self.sig_win.canvas.x_mode == 'lambda' else np.arange(len(band_cols))
        )

        if self.sig_win.chk_all_dates is not None and self.sig_win.chk_all_dates.isChecked():
            fechas = list(self.df_list['Fecha'].astype(str).unique())
        else:
            fechas = [self.active_date] if self.active_date is not None else []

        for px in self.selected_pixels:
            for fecha in fechas:
                subset = self.df_list[self.df_list['Fecha'].astype(str) == str(fecha)]
                if subset.empty:
                    continue
                ...
                rows.append((px.pid, px.i, px.j, str(fecha)))
    ...
    def _open_mcal_sig_window(self):
        self.mcal_sig_win.load_groups_and_dates()
    ...
    def _export_csv(self):
        ...
        band_cols = get_band_sequence(self.sig_win.get_selected_band_sequence_name())
        lambda_vals = get_lambda_for_sequence(self.sig_win.get_selected_band_sequence_name())
        ...
        for px in self.selected_pixels:
            for fecha in fechas:
                ...
                try:
                    ...
                    vals = np.array([vals[BAND_NAMES.index(b)] for b in band_cols], dtype=float)
                except Exception:
                    continue
                for b_idx, band_name in enumerate(band_cols):
                    rows.append({
                        ...
                        'Fecha': fecha,
                        'band': band_name,
                        'lambda_nm': float(lambda_vals[b_idx]) if b_idx < len(lambda_vals) else np.nan,
    ...
    def _save_project(self):
        ...
        state = ProjectState(
            ...
            pixel_xaxis_mode='lambda' if self.sig_win.chk_lambda.isChecked() else 'band',
            pixel_band_sequence=self.sig_win.get_selected_band_sequence_name(),
            pixel_all_dates=bool(self.sig_win.chk_all_dates.isChecked()) if self.sig_win.chk_all_dates is not None else False,
        )
    ...
    def _load_project(self):
        ...
        try:
            ...
            sig_mode = data.get('pixel_xaxis_mode', 'lambda')
            self.sig_win.chk_lambda.setChecked(sig_mode == 'lambda')
            sig_seq = data.get('pixel_band_sequence', 'Nband_sort')
            if sig_seq in get_available_band_sequence_names():
                self.sig_win.cmb_band_sequence.setCurrentText(sig_seq)
            if self.sig_win.chk_all_dates is not None:
                self.sig_win.chk_all_dates.setChecked(bool(data.get('pixel_all_dates', False)))
