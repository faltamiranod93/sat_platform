"""
Genera, por cada fecha del ROI, un classmap raster + dos capas de calidad (confianza y similitud máxima) + una vista PNG;
...
archivo_mcal_mod = f'{path}/McalHSL_mod_v4_py.csv'
...
# Agregar HSL a Mcal
Rg = McalHSL_mod.reindex(columns=Nband_sort)  # Reordenar columnas de bandas
hsl_rg = pd.DataFrame(rgb2hsl(Rg.iloc[:, [3, 2, 1]] / 10000.), columns=['H', 'S', 'L'])
# Escalar los valores de H, S y L
hsl_rg['H'] = hsl_rg['H'] * (10000 / 360)
hsl_rg['S'] = hsl_rg['S'] * (10000 / 100)
...
Mcal_HSL = McalHSL_mod.copy()
Mcal_HSL[['H','S','L']] = hsl_rg
#Mcal_HSL = McalHSL_mod.join(hsl_rg, how='outer')

# Convertir la columna Fecha al formato adecuado
Mcal_HSL['Fecha'] = pd.to_datetime(Mcal_HSL['Fecha'], errors='coerce').dt.strftime('%Y-%m-%d')

group = np.unique(McalHSL_mod["Ng"])
Ng = len(group)
MR_ref  = np.zeros((Ng,Nlam+3))
...
for fecha in df_roi_list['Fecha'].unique():
    ...
    clases_unicas = np.unique(ico_image)
    mask = np.zeros((Ni,Nj,Ng))
    for clase in clases_unicas:
        mask [:,:,clase] = (ico_image == clase).astype(np.uint8)
    # Combinar las máscaras de clases específicas
    combined_mask = np.sum(mask[:, :, [3, 4, 5]], axis=2)    
    # Expandir aux para que coincida con las dimensiones de refle_data
    combined_mask_expanded = np.repeat(combined_mask[:, :, np.newaxis], Nlam + 3, axis=2).reshape(Ni * Nj, Nlam + 3)
    #_________________________
    
    refle_data_masked = refle_data * combined_mask_expanded
    
    if refle_data_masked.shape[0] == 0:
        print('f"No hay datos para la segunda clasificación en {fecha}. Saltando..."')
        
    # Inicializar matriz de distancias
    Mco_lam_2 = np.full((len(refle_data_masked), Ng), np.inf)
    
    # Calcular distancias espectrales
    for jg in range(3,6): # En esta parte ocurre la magia al parecer
        distancias_ref = np.sum((refle_data_masked - MR_ref_mod[jg])**2, axis=1)
        Mco_lam_2[:, jg] = np.minimum(Mco_lam_2[:, jg], distancias_ref)
    
    # Reclasificar píxeles por distancia espectral
    ico_2 = np.argmin(Mco_lam_2, axis=1)
    ico_2_image = ico_2.reshape((Ni, Nj))
    
    # Combina ico_image con ico_2_image
    new_ico_image = np.sum(ico_image[:, :, np.newaxis] * mask[:, :, [0, 1, 2, 6]], axis=2) + ico_2_image * combined_mask
    
    colored_class_map = np.zeros((Ni, Nj, 3))
    for clase in np.unique(new_ico_image):
        clase = int(clase)  # Asegura que sea un entero
        if 0 <= clase < len(color):
            mask_clase = (new_ico_image == clase)
            for k in range(3):  # R, G, B
                colored_class_map[:, :, k][mask_clase] = color[clase, k]
    
    rgb_ima = np.zeros((Ni,Nj,3))
    rgb_ima[:,:,0] = rgb_refle[:,0].reshape(Ni,Nj)
    rgb_ima[:,:,1] = rgb_refle[:,1].reshape(Ni,Nj)
    rgb_ima[:,:,2] = rgb_refle[:,2].reshape(Ni,Nj)
    rgb_image = (rgb_ima* 255).astype(np.uint8)
    #_________________________

    # Clasificación coloreada de mapa
    output_filename_class_map = f'{fecha}_{name}_class_map_{ver}.png'
    output_file_class_map = os.path.join(class_dir,output_filename_class_map)
    img_save = (colored_class_map * 255).astype(np.uint8)
    if not os.path.exists(output_file_class_map):
        Image.fromarray(img_save).save(output_file_class_map)
        print(f"Clasificación guardada en {output_file_class_map}")
    else:
        print(f'El archivo ya existe, se omite el guardado: {output_file_class_map}')
    ...
    if not os.path.exists(output_file_class):
        ...
        dataset_class.GetRasterBand(1).WriteArray(new_ico_image)
...
for jg in [0,1,2,3,4,5,6,7]:
    plt.plot(lam_sorted, MR_ref_mod[jg, :Nlam], #MR_ref_mod_norm
             label=f'{nameg_eng[jg]} (Modified)',
             linestyle='-',
             color=colores_grupos[jg])

# Configuración de la leyenda
plt.legend(
    title="Spectral Groups",        # Título de la leyenda
...
plt.title('Mean Reflectance by Group')
...
# Subplot 1: Reflectancia normalizada
for jg in [0,1,2,3,4,5,6,7]:
    axes[0].plot(lam_sorted, MR_ref_mod_norm[jg, :Nlam],
                 label=f'{nameg_eng[jg]} (Modified)',
                 color=colores_grupos[jg])
axes[0].set_title('Mean Reflectance Normalized by Group')
axes[0].set_xlabel('Wavelength (nm)')
axes[0].set_ylabel('Mean Reflectance Normalized')
axes[0].grid(True)

# Subplot 2: Reflectancia original
for jg in [0,1,2,3,4,5,6,7]:
    axes[1].plot(lam_sorted, MR_ref_mod[jg, :Nlam],
                 label=f'{nameg_eng[jg]} (Modified)',
                 color=colores_grupos[jg])
axes[1].set_title('Mean Reflectance by Group')
axes[1].set_xlabel('Wavelength (nm)')
axes[1].set_ylabel('Mean Reflectance')
axes[1].grid(True)

# Leyenda en el segundo subplot
axes[1].legend(
...
# Suponiendo que tienes una lista de fechas procesadas
fechas = df_roi_list['Fecha'].unique()

# Crear figura con subplots (por ejemplo, 3 columnas)
n = len(fechas)
ncols = 4
...
for i, fecha in enumerate(fechas):
    ...
    # Máscaras por clase
    mask = np.zeros((Ni, Nj, Ng))
    for clase in np.unique(ico_image):
        mask[:, :, clase] = (ico_image == clase).astype(np.uint8)

    combined_mask = np.sum(mask[:, :, [0, 1]], axis=2)  # combinar clase 1 y 2

    # Graficar
    ax = axes[i]
    #ax.imshow(combined_mask[240:440,300:500], cmap='binary')
    ax.imshow(mask[:,:,7], cmap='binary')
    ax.set_title(f"Fecha: {fecha}")
    ax.axis('off')

# Ocultar ejes vacíos si hay más subplots que fechas
