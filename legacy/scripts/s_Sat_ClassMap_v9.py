...
por distancia de Mahalanobis usando features = Nband_filter + HSL.
...
def build_features_from_cube(cube: np.ndarray, band_to_idx: dict, bands_filter: list[str]) -> np.ndarray:
    ...
    HSL = (np.stack([h, s, l], axis=1) * 10000.0).astype(dtype_work, copy=False)
...
def main():
    ...
    feature_cols = list(Nband_filter)
    ...
    for fecha in df_roi_list['Fecha'].unique():
        ...
        if nan_count > 0 or inf_count > 0:
            raise ValueError(f'Fecha {fecha}: {nan_count} NaN y {inf_count} Inf en features')
        ...
        # second best (eficiente con argsort parcial)
        sorted_idx = np.argsort(d2_all, axis=1)
        second_idx = sorted_idx[:, 1]  # 2nd smallest
        second_d2 = d2_all[np.arange(P), second_idx]
        ...
        write_geotiff(out_class_tif, label_img, geotransform, projection, gdal.GDT_Int16, nodata=0)
    ...
    merged = merged.drop_duplicates(subset=['Fecha', 'Ver Class'], keep='last')
