...
class QuicklookExporterPort(Protocol):
    """Exporta PNG/JPEG a partir de 1 o 3 rasters (ej. RGB o classmap colorizado)."""
    def export_rgb(self, r: GeoRaster, g: GeoRaster, b: GeoRaster, out_uri: URI, spec: QuicklookSpec = QuicklookSpec()) -> URI: ...
    def export_gray(self, x: GeoRaster, out_uri: URI, spec: QuicklookSpec = QuicklookSpec()) -> URI: ...
    def export_classmap(self, labels: GeoRaster, classes: Sequence[ClassLabel], out_uri: URI, spec: QuicklookSpec = QuicklookSpec()) -> URI: ...
